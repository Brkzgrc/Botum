...
def get_tradable_symbols():
    ...
    try:
        if not MARKETS_LOADED:
            print("-> Marketler yükleniyor...", flush=True)
        ...
        print(f"⚠️ Market Hatası: {e}", flush=True)
        return []
...
def get_macro_regime(symbol):
    try:
        ...
        df_1h = get_data(MACRO_SYMBOL, '1h', limit=100)
        if df_1h is None:
            return "NEUTRAL", None
...
def find_structural_target(df_15m, entry_price, coin_type="NORMAL"):
    try:
        lookback = 50
        swing_high = df_15m['high'].iloc[-lookback:-1].max()
        structural_target = entry_price * 1.03 if swing_high <= entry_price * 1.005 else swing_high
...
def strategy_sfp_dynamic(df_15m):
    try:
        ...
        vol_ratio = 1.0 if pd.isna(atr_mean) or atr_mean == 0 else atr_now / atr_mean
        ...
        if vol_ratio >= 1.25:
            ...
            coin_type_tag = "🧊 CALM"
            sweep_mult = 0.10; reclaim_mult = 0.20; stop_mult = 0.25
        ...
        if swept and reclaimed and strong_wick and vol_ok:
            ...
            final_type = "🟢 SFP-A (GOLD)" if is_gold else "🟡 SFP-B (SILVER)"
            desc = "Dip Süpürme + RSI Uyumu" if is_gold else "Dip Süpürme (Standart)"
            
            return True, {
                'type': f"{final_type} | {coin_type_tag}",
                'desc': desc,
                'stop': safe_stop,
...
def strategy_breakout(df_15m):
    try:
        last = df_15m.iloc[-1]
        upper = last['upper_band']; vol_ma = last['vol_ma']
        if pd.isna(upper): return False, None
        
        breakout = last['close'] > upper
...
def strategy_bomb_candidate(df_15m):
    try:
        if len(df_15m) < 120: return False, None
        
        last = df_15m.iloc[-1]
        if last['close'] < last['ema200']: return False, None
        
        atr = last['atr']; atr_mean = last['atr_mean']
        if pd.isna(atr_mean) or atr_mean == 0: return False, None
        
        compression = atr / atr_mean
        if compression > 0.75: return False, None
        
        if not (55 <= last['rsi'] <= 68): return False, None
        
        if last['volume'] < last['vol_ma'] * 1.5: return False, None
        
        recent_high = df_15m['high'].iloc[-20:-1].max()
        if last['close'] > recent_high * 1.03: return False, None
        
        stop = last['low'] - (1.2 * atr)
        
        return True, {
            'type': '💣 BOMB CANDIDATE',
            'desc': 'ATR Sıkışma + Hacim Öncü Artış (1–4 Saat)',
            'stop': stop,
            'coin_type': 'BOMB'
        }
    except:
    ...
    while True:
        try:
            ...
            for symbol in symbols:
                count += 1
                if count % 10 == 0:
                    print(f"-> {count}/{total} ({symbol})", flush=True)

                try:
                    if symbol in signal_history:
                        continue

                    regime, _ = get_macro_regime(symbol)
                    if regime == "DOWNTREND":
                        continue
                    ...
                    if signal_found:
                        ...
                        tp_price, tp_pct = find_structural_target(df_15m, entry_price, data.get('coin_type', 'NORMAL'))
                        ...
                        signal_time = tr_time.strftime('%H:%M')
                        msg = f"""
<b>{data['type']}</b>
━━━━━━━━━━━━━━━━━━━━
<b>#{symbol}</b> | 🕒 {signal_time}
━━━━━━━━━━━━━━━━━━━━
🧠 <b>REJİM:</b> {regime}
📝 <b>NEDEN:</b> {data['desc']}
...
🎯 <b>HEDEF</b>
━━━━━━━━━━━━━━━━━━━━
🏆 <code>{tp_price:.4f}</code> | Potansiyel %{tp_pct:.2f}
"""
                        try:
                            requests.post(
            ...
            print("🏁 Tarama Bitti — 5 dk bekleniyor", flush=True)
            time.sleep(300)

        except Exception as e:
            print(f"🔥 ANA HATA: {e}", flush=True)
            time.sleep(60)
...
if __name__ == "__main__":
    ...
    print("🌍 Web panel aktif", flush=True)
    run_bot_engine()
