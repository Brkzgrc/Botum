...
L2_DELAY_SEC = 55  # Katman-2 gecikmesi (coin başı 55 sn)
BAN_UNTIL_TS = 0
...
# ✅ DÜZELTİLMİŞ: EMA200 için yeterli veri (limit=260)
...
# ✅ DÜZELTİLMİŞ: Sadece GOLD çalışıyor (SILVER KALDIRILDI)
...
# ✅ SFP: Sadece GOLD çalışıyor (SILVER KALDIRILDI)
def strategy_sfp_dynamic(df_15m):
    try:
        ...
        if swept and reclaimed and strong_wick and vol_ok:
            ...
            # ✅ Sadece GOLD kabul edilir (SILVER reddedilir)
            if is_gold:
...
def run_bot_engine():
    ...
    while True:
        try:
            ...
            candidates = []  # Katman-1'de bulunan adaylar
            ...
            for symbol in symbols:
                ...
                try:
                    ...
                    # ✅ Sadece 3 strateji çalışıyor (Bomb/Squeeze/SILVER KALDIRILDI)
                    signal_found = False
                    data = {}
            ...
            if candidates:
                for idx, (symbol, data, df_15m, coin_regime, macro_regime) in enumerate(candidates, 1):
                    ...
                    time.sleep(L2_DELAY_SEC)  # Coin başı gecikme
