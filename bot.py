...
# --- 3. VERİ MOTORU (ANAYASA + 5 MADDE TAM ENTEGRASYON) ---
def get_data(symbol, interval):
    try:
        ...
        # ADX (Madde 2: Trend Gücü - Tam Hesaplama)
        ...
        # Fibonacci (Madde 4: Hedef Seviyesi Radar)
...
def scan():
    ...
    try:
        ...
        print(f"📡 {len(symbols)} coin analiz ediliyor...", flush=True)
            ...
            if score >= 9:
                ...
                if s not in sent_signals or (time.time() - sent_signals[s]) > 14400:
                    ...
                    msg = (f"⭐ **{signal_type}**\n\n"
                           ...
                           f"🎯 Hedef: {t_profit:.8f}\n"
                           f"📉 Fib Radar: {fib_inf}\n"
                           f"🐳 Whale: x{whale:.1f} | ADX: {last['adx']:.1f}\n"
                           f"🧡 BTC: %{btc_change:.2f}\n\n"
                    ...
                    print(f"✅ Sinyal Gönderildi: {s}", flush=True)
            time.sleep(0.05)
