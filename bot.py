...
# ============================================================
# 1) AYARLAR (ANAHTARLAR ORTAM DEĞİŞKENLERİNDEN OKUNUR)
# ============================================================
...
EXPLAIN_SIGNALS = True         # Telegram kartına kriter detaylarını ekle
...
# ============================================================
# 2) BAN ÖNLEME / RATE LIMIT KATMANI (KRİTİK)
# ============================================================
class ApiGate:
    """
    ...
    """
    def __init__(self, min_interval_sec=0.18, max_concurrent=1, max_retries=6):
...
api_gate = ApiGate(
    min_interval_sec=float(os.environ.get("API_MIN_INTERVAL_SEC", 0.18)),
...
exchange = ccxt.binance({
    ...
    'enableRateLimit': True,   # ccxt'nin kendi limiter'ı
    'timeout': 15000
})
...
# ============================================================
# 5) 1H TREND CONFIRMATION (KURAL AYNI, VERİ LİMİTİ DÜZELTİLDİ)
# ============================================================
def is_1h_trend_aligned(symbol):
    """
    15m sinyali için 1h yükseliş trendi zorunlu kontrolü.
    EMA50 > EMA200 ve fiyat EMA50 üstünde olmalı.
    """
    try:
        df_1h = get_data(symbol, '1h', limit=260)  # EMA200 için gerekli
...
# ============================================================
# 6) POZİSYON HESAPLAMA (%2 RİSK BAZLI)
# ============================================================
...
# ============================================================
# 7) TICKER CACHE (BAN ÖNLEME: fetch_ticker yerine fetch_tickers)
# ============================================================
ticker_cache = {
    "ts": 0.0,
    "data": {},   # symbol -> ticker dict
}
TICKER_CACHE_SEC = 20

def refresh_tickers(symbols):
    """
    Binance spot: fetch_tickers destekler.
    - Tek çağrıyla çok sayıda sembolün last/quoteVolume alınır.
    - Eğer bazı hesaplarda destek/limit sorunu olursa chunk'lar.
    """
    now = time.time()
    if (now - ticker_cache["ts"]) < TICKER_CACHE_SEC and ticker_cache["data"]:
    ...
    for i in range(0, len(symbols), CHUNK):
        ...
        try:
            res = api_gate.call(exchange.fetch_tickers, part)
            if isinstance(res, dict):
                out.update(res)
        except Exception as e:
            print(f"⚠️ fetch_tickers chunk hata: {str(e)[:120]}", flush=True)
            # chunk patlarsa, en azından devam et (ban önleme adına)
            continue

    if out:
        ticker_cache["ts"] = now
        ticker_cache["data"] = out
    return ticker_cache["data"]

def get_last_price_from_cache(symbol: str):
    t = ticker_cache["data"].get(symbol)
    if not t:
        return None
    return t.get("last", None)

def get_liquidity_warning_from_cache(symbol):
    t = ticker_cache["data"].get(symbol)
    if not t:
...
# ============================================================
# 8) EXPLAIN BLOCK (ÇÖKME HATASI DÜZELTİLDİ - SİNYALİ ETKİLEMEZ)
# ============================================================
def build_explain_block(symbol: str, df_15m: pd.DataFrame, data: dict) -> str:
    try:
        ...
        _ = upper_band  # ileride kullanılabilir
        ...
        if "SFP" in stype and "GOLD" in stype:
            ...
            req_lines.append(f"✅ RSI uyumsuzluğu var (GOLD sinyali) • Pivot RSI={_fmt_num(pivot_rsi,2)} | Şimdi RSI={_fmt_num(rsi,2)}")
        ...
        # ---------------- PULLBACK ----------------
            ...
            req_lines.append(f"{_yn(trend_ok)} Genel trend pozitif (EMA50 > EMA200)")
        ...
        # ---------------- RE-ACCUMULATION ----------------
            ...
            req_lines.append(f"{_yn(trend_ok)} Trend sağlam (fiyat ve ortalamalar yukarı)")
            req_lines.append(f"• Fiyat={fmt_price(symbol, close)} | EMA50={fmt_price(symbol, ema50)} | EMA200={fmt_price(symbol, ema200)}")
            req_lines.append(f"{_yn(rsi_ok)} Momentum dengeli (RSI uygun)")
            req_lines.append(f"• RSI={_fmt_num(rsi,2)}")
            req_lines.append(f"{_yn(range_ok)} Fiyat dar aralıkta (bayrak/kanal)")
...
def home():
    ...
    return f"""
    <h1>🚀 Sniper Bot Kontrol Paneli (BAN-SAFE v2.1)</h1>
    ...
    <hr>
    <p><b>ÖNEMLİ:</b> Aktif stratejiler: PULLBACK + RE-ACCUMULATION + SFP-GOLD</p>
    <p><b>Risk Ayarı:</b> %2 risk/trade ({ACCOUNT_SIZE}$ için $100 maks risk)</p>
    <p><b>Filtre:</b> 1h trend confirmation ZORUNLU</p>
    <p><b>Ban-Safe:</b> Toplu ticker + rate limiter + retry/backoff</p>
    """
...
def send_test_message():
    try:
        r = requests.post(
            ...
            json={"chat_id": CHAT_ID, "text": "✅ Bot ayakta: BAN-SAFE v2.1 test mesajı", "parse_mode": "HTML"},
            timeout=10
        )
...
# ============================================================
# 15) ANA MOTOR (BAN-SAFE: çağrı azaltma)
#     - 15m -> strateji adayı -> (sonra) 1h trend + RS
# ============================================================
def run_bot_engine():
    print("🚀 Sniper Bot Motoru BAŞLATILDI (BAN-SAFE v2.1)", flush=True)
    ...
    while True:
        try:
            ...
            total = len(symbols)
            if total == 0:
                print("⚠️ Sembol listesi boş. 60 sn bekleniyor...", flush=True)
                time.sleep(60)
                continue

            # Ban-safe: toplu ticker yenile (last + quoteVolume)
            refresh_tickers(symbols + [MACRO_SYMBOL])
            ...
            to_remove = [
                k for k, t in list(signal_history.items())
            ...
            # BTC 15m (RS filtresi için) -> bir kez
            ...
            for symbol in symbols:
                ...
                try:
                    ...
                    # --- BAN-SAFE: artık aday çıktı, şimdi 1H trend kontrolü ---
                    is_aligned, trend_msg = is_1h_trend_aligned(symbol)
                    if not is_aligned:
                        reject["1h_trend_mismatch"] += 1
                        continue

                    # --- BAN-SAFE: aday çıktıysa RS filtresi ---
                    ...
                    # --- Fiyat/likidite (cache) ---
                    last_price = get_last_price_from_cache(symbol)
                    ...
                    liquidity_warning = get_liquidity_warning_from_cache(symbol)
...
💰 <b>POZİSYON ÖNERİSİ ({ACCOUNT_SIZE}$ sermaye, %{RISK_PERCENT} risk):</b>
   ...
   • Gerçek Risk: %{actual_risk_pct:.2f} | RR Oranı: 1:{rr_ratio:.2f}
📝 <b>NEDEN:</b> {data['desc']}
{explain_block}
""".strip()
...
if __name__ == "__main__":
    # Mini kontrol: anahtarlar yoksa uyar
    ...
    print("✅ Ban-Safe: toplu ticker + rate limiter + retry/backoff", flush=True)
