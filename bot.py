# -*- coding: utf-8 -*-
"""
Sniper Bot v2.0 - Gerçek Pump'ları Yakalayan Versiyon
"""
...
# ============================================================
# 0) KULLANICI AYARLARI - YENİ PARAMETRELER
# ============================================================
BINANCE_API_KEY = ""
BINANCE_API_SECRET = ""
...
# --- FİLTRE / PARAMETRE (GEVŞETİLDİ) ---
MIN_ATR_PCT = 0.0025              # 0.0030 → 0.0025 (daha fazla coin geçer)
MAX_SPREAD_PCT = 0.25             # 0.15 → 0.25 (likidite düşük coinler geçer)
MIN_RR = 1.2                      # 1.5 → 1.2 (daha kolay geçer)

# Cooldown'lar
COOLDOWN_MINUTES = 90             # 120 → 90
SFP_COOLDOWN_MIN = 45             # 60 → 45
PULLBACK_COOLDOWN_MIN = 240       # 360 → 240
REACCU_COOLDOWN_MIN = 360         # 480 → 360
REVERSAL_COOLDOWN_MIN = 120       # YENİ
BREAKOUT_COOLDOWN_MIN = 120       # YENİ
MOMENTUM_COOLDOWN_MIN = 90        # YENİ
...
RS_BARS_12H = 48
RS_BARS_24H = 96
RS_MIN_REL_1H  = 0.08             # 0.15 → 0.08
RS_MIN_REL_4H  = 0.25             # 0.50 → 0.25
RS_MIN_SCORE   = 0.35             # 0.60 → 0.35
RS_RISK_OFF_MIN_SCORE = 1.00      # 1.50 → 1.00
...
def find_structural_target(df_15m: pd.DataFrame, entry_price: float):
    try:
        ...
        return entry_price * 1.03, 3.0, "Fallback (%3)", None
      # ============================================================
# 8) STRATEJİLER - ESKİLER + 3 YENİ
# ============================================================

# ESKİ STRATEJİLER (AYNI)
...
# ============================================================
# YENİ STRATEJİ 1: REVERSAL (Dip Toparlanma - Sarı Mumlar)
# ============================================================
def strategy_reversal(df_15m):
    """
    Düşüş sonrası güçlü toparlanma (V-şekli)
    ZEC, QNT, SUSDT'deki sarı mumlar gibi
    """
    try:
        if len(df_15m) < 60:
            return False, None
        ...
        # 1) Yakın zamanda düşüş var mı? (Son 20 mum)
        ...
        if current_close > (recent_low * 1.10):  # Dipten %10 üstündeyse red
            return False, None
        
        # 3) Güçlü yeşil mum (toparlanma)
        body = abs(float(last["close"]) - float(last["open"]))
        ...
        # 4) Hacim patlaması (toparlanmada hacim olmalı)
        vol_ma = float(last["vol_ma"]) if not pd.isna(last["vol_ma"]) else 0.0
        if vol_ma == 0:
            return False, None
        
        vol_ratio = float(last["volume"]) / vol_ma
        if vol_ratio < 1.8:  # En az 1.8x hacim
            return False, None
        
        # 5) RSI dip bölgesinden çıkıyor mu?
        rsi = float(last["rsi"])
        if pd.isna(rsi):
            return False, None
        
        # RSI 30-50 arası (oversold'dan çıkıyor)
        if not (25 <= rsi <= 55):
            return False, None
        
        # Stop: Son düşüşün dibi
        stop = recent_low * 0.98
        
        return True, {
            "type": "💎 REVERSAL (V-Dip)",
            "desc": f"Dipten toparlanma | Düşüş: %{decline:.1f} | Hacim: {vol_ratio:.1f}x",
            "stop": stop,
            "coin_type": "REVERSAL",
        }
...
# ============================================================
# YENİ STRATEJİ 2: BREAKOUT (Range Kırılımı)
# ============================================================
def strategy_breakout(df_15m):
    """
    Konsolidasyon sonrası breakout
    ZEC'nin range üstüne çıkışı gibi
    """
    try:
        if len(df_15m) < 60:
            return False, None
        ...
        # 1) Konsolidasyon var mı? (Son 30 mumda dar range)
        lookback = 30
        recent = df_15m.iloc[-lookback:-1]
        ...
        # Range %5-20 arası olmalı (çok dar veya çok geniş değil)
        if not (5 <= range_pct <= 20):
            return False, None
        
        # 2) Fiyat range üstüne çıktı mı?
        current_close = float(last["close"])
        breakout = current_close > (range_high * 1.002)  # %0.2 üstünde
        ...
        if not (is_green and body_ratio > 0.55):
            return False, None
        ...
        # 5) RSI overbought değil
        rsi = float(last["rsi"])
        if pd.isna(rsi) or rsi > 72:
            return False, None
        
        # Stop: Range orta noktası
        stop = range_mid * 0.99
        
        return True, {
            "type": "⚡ BREAKOUT",
            "desc": f"Range kırılımı | Range: %{range_pct:.1f} | Hacim: {vol_ratio:.1f}x",
            "stop": stop,
            "coin_type": "BREAKOUT",
        }
...
# ============================================================
# YENİ STRATEJİ 3: MOMENTUM (Trend Devamı)
# ============================================================
def strategy_momentum(df_15m):
    """
    Güçlü yükseliş trendi + yeni ATH
    Sürekli yükselen coinler için
    """
    try:
        if len(df_15m) < 60:
            return False, None
        ...
        # 1) Yükseliş trendi (EMA50 > EMA200 ve yükseliyor)
        ema50 = float(last["ema50"])
        ema200 = float(last["ema200"])
        ema50_prev = float(df_15m["ema50"].iloc[-4])
        
        if pd.isna(ema50) or pd.isna(ema200) or pd.isna(ema50_prev):
            return False, None
        
        if not (ema50 > ema200 and ema50 > ema50_prev):
            return False, None
        
        # 2) Yeni yüksek yapıyor mu? (Son 40 mumun en yükseği)
        recent_40 = df_15m.iloc[-40:-1]
        prev_high = float(recent_40["high"].max())
        current_close = float(last["close"])
        
        # Yeni ATH değil ama yakın zamanda yeni yüksek
        if current_close <= prev_high * 0.995:
            return False, None
        ...
        vol_ratio = float(last["volume"]) / vol_ma
        if vol_ratio < 1.3:  # Momentum'da daha düşük hacim yeterli
            return False, None
        ...
        if not (55 <= rsi <= 75):  # Güçlü momentum bölgesi
            return False, None
        
        # 6) ATR genişlemeli (volatilite artışı)
        atr = float(last["atr"])
        atr_mean = float(last["atr_mean"]) if not pd.isna(last["atr_mean"]) else 0.0
        ...
        atr_expansion = atr / atr_mean
        if atr_expansion < 1.1:
            return False, None
        
        # Stop: Son 10 mumun en düşüğü veya EMA50
        recent_10 = df_15m.iloc[-10:-1]
        swing_low = float(recent_10["low"].min())
        stop = max(swing_low, ema50 * 0.98)
        
        return True, {
            "type": "🚀 MOMENTUM",
            "desc": f"Trend devamı + Yeni yüksek | ATR: {atr_expansion:.2f}x",
            "stop": stop,
            "coin_type": "MOMENTUM",
        }
...
def build_explain_block(df_15m: pd.DataFrame, data: dict) -> str:
    ...
    try:
        ...
        elif "REVERSAL" in stype or "BREAKOUT" in stype or "MOMENTUM" in stype:
            lines.append(f"• RSI={_fmt_num(rsi,2)} | ATR={_fmt_num(compression,2)}x | VolGüç={_fmt_x(vol_strength,2)}")
        else:
            return ""
        return "\n".join(lines)
    except Exception:
...
# ============================================================
# 14) ADAY ÜRETİMİ - YENİ STRATEJİLERLE
# ============================================================
async def evaluate_symbol_on_close(symbol: str, df_15m: pd.DataFrame, candidate_queue: asyncio.Queue):
    ...
    try:
        ...
        now_ts = time.time()
        if now_ts - btc_15m_cache["ts"] > 900:
            try:
        ...
        # STRATEJİ SEÇİMİ - YENİ STRATEJİLER ÖNCELİKLİ
        signal_found = False
        data = None

        # Önce yeni stratejiler (daha agresif)
        ok, d = strategy_reversal(df_15m)
        if ok:
            signal_found = True; data = d
        
        if not signal_found:
            ok, d = strategy_breakout(df_15m)
            if ok:
                signal_found = True; data = d
        ...
        elif "REVERSAL" in stype:
            cooldown_min = REVERSAL_COOLDOWN_MIN
        elif "BREAKOUT" in stype:
            cooldown_min = BREAKOUT_COOLDOWN_MIN
        elif "MOMENTUM" in stype:
    ...
    while True:
        ...
        try:
            ...
            if not is_ok:
                ...
                print(f"  ❌ {symbol}: {trend_msg}", flush=True)
                continue
            ...
            if not spread_ok:
                stats["spread_red"] += 1
                print(f"  ❌ {symbol}: {spread_msg}", flush=True)
                continue
            ...
            try:
                ...
                print(f"  ⚠️ {symbol}: Ticker hatası - {str(e)[:80]}", flush=True)
...
""".strip()
            ...
            print(f"⚠️ Worker hatası ({symbol}): {str(e)[:160]}", flush=True)
        finally:
...
async def main():
    print("🚀 Bot v2.0 başladı - Yeni stratejilerle!", flush=True)
