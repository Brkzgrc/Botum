...
def health_check():
    ...
    return f"SMC Momentum v7 — LuxAlgo | {boot_status} | {cached} coin cached", 200
...
# 1c) BTC TREND
# ============================================================
btc_trend_cache = {"trend": "UNKNOWN", "close": 0, "updated": 0, "momentum": "UNKNOWN"}
...
def refresh_btc_trend():
    try:
        ...
        if len(bars_1h) >= 25:
            ...
            # EMA8 > EMA21 ve ikisi de yükseliyor = UP
            # EMA8 < EMA21 = DOWN (kısa vadeli düşüş başlamış)
...
def analyze(symbol):
    try:
        ...
        if btc_trend == "BEAR":
            ...
            return
        # Momentum filtre: BTC 1H EMA8 < EMA21 ise sinyal verme
        if btc_momentum == "DOWN":
            scan_stats["btc_momentum_skip"] += 1
            return
...
def start_scanner():
    ...
    print("=" * 50)
    print("🚀  SMC Momentum v7 — BTC Kısa Vadeli Momentum Filtreli")
    print("=" * 50)
    ...
    print(f"  Momentum       : BTC 1H EMA8 < EMA21 ise sinyal üretilmez")
    print(f"  Discount Zone  : LuxAlgo birebir (alt %5 bant)")
    print(f"  Source         : smc-momentum")
    print("=" * 50 + "\n")
    ...
    while True:
        ...
        elif btc_trend_cache.get("momentum") == "DOWN":
            print(f"⚠️ BTC Momentum DOWN — sinyal üretilmiyor.")
        else:
        ...
        if btc_trend != "BEAR":
            print(f"Momentum     : {btc_trend_cache.get('momentum', '?')}", flush=True)
            print(f"Mom. skip    : {scan_stats.get('btc_momentum_skip', 0)}", flush=True)
