...
# Bazı sistemlerde BINANCE_SECRET_KEY, bazılarında BINANCE_API_SECRET kullanılıyor.
...
# ✅ Katman-2: aday coin başı bekleme (rate-limit kontrol)
L2_DELAY_SEC = 55

# ✅ Aynı taramada max kaç aday Katman-2’ye gidebilir
...
# ============================================================
# 4) 15m KAPANIŞINA KİLİT
# ============================================================
def wait_until_next_15m_close():
    """15m mum kapanışını bekle (UTC bazlı - Binance standardı)"""
    while True:
...
# ============================================================
# 11) EXPLAIN BLOCK (senin sürümden aynı mantık)
# ============================================================
...
# ============================================================
# 12) FLASK / WATCHDOG / HEARTBEAT
# ============================================================
app = Flask(__name__)
signal_history = {}
bomb_history = {}  # ✅ sadece symbol bazlı tutacağız
...
def home():
    ...
    return f"""
    <h1>🚀 Sniper Bot v5.1 (Katman-1 Aday + Katman-2 Yavaş Doğrulama)</h1>
    ...
    <p><b>BTC Cache TTL:</b> ✅ 15 dakika</p>
    <p><b>Tarama:</b> ✅ 15m mum kapanışında</p>
    <p><b>Katman-2 Delay:</b> ✅ {L2_DELAY_SEC}s / aday</p>
    """
...
# ============================================================
# 13) ANA MOTOR (Katman-1 + Katman-2)
# ============================================================
def run_bot_engine():
    print("🚀 Sniper Bot v5.1 BAŞLATILDI (Katman-1 + Katman-2)", flush=True)
    bot_status["status"] = "Aktif"

    # İlk tur: 15m kapanışa kilit
    ...
    while True:
        try:
            ...
            print(f"✅ L1 BAŞLADI | toplam={len(symbols)}", flush=True)

            reject = defaultdict(int)
            sent_by_type = defaultdict(int)
            candidates = []

            # History temizliği (en uzun cooldown kadar)
            ...
            for symbol in symbols:
                ...
                heartbeat["progress"] = f"L1 {count}/{total}"
                beat()

                if count % 20 == 0:
                    print(f"-> L1 ilerleme: {count}/{total} ({symbol})", flush=True)
                ...
                try:
                    ...
                    # Strateji seçimi (sadece "setup var mı?" diye bak)
                    signal_found = False
                    data = {}

                    if coin_regime in ["RANGING", "NEUTRAL"]:
                        # Bomb adayını L1'de de bulabiliriz ama asıl cooldown+send L2’de
                    ...
                    # L1: sadece aday listeye al
                    if len(candidates) >= MAX_CANDIDATES_PER_SCAN:
                        reject["candidate_cap"] += 1
                        continue
                    ...
                    print(f"⚠️ L1 Hata ({symbol}): {e}", flush=True)
                    continue

            print(f"\n🧲 L1 BİTTİ | aday={len(candidates)} (cap={MAX_CANDIDATES_PER_SCAN})", flush=True)

            # -------------------------
            # KATMAN-2: YAVAŞ DOĞRULA + GÖNDER
            # -------------------------
            heartbeat["progress"] = f"L2_START | aday={len(candidates)}"
            heartbeat["last_symbol"] = None
            beat(force_print=True)
            ...
            for i, c in enumerate(candidates, start=1):
                ...
                data = c["data"]
                df_15m = c["df_15m"]
                coin_regime = c["coin_regime"]
                macro_regime = c["macro_regime"]
                rs_score = c.get("rs_score", np.nan)
                rs_1h = c.get("rs_1h", np.nan)
                rs_4h = c.get("rs_4h", np.nan)

                heartbeat["last_symbol"] = symbol
                heartbeat["progress"] = f"L2 {i}/{len(candidates)}"
                beat()

                # ✅ her adaydan önce bekle: rate-limit/bana karşı asıl kalkan
                if i > 1:
                    time.sleep(L2_DELAY_SEC)
                ...
                try:
                    ...
                    # 3) Cooldown / Bomb cooldown
                    stype = (data.get('type', 'UNKNOWN') or "UNKNOWN").upper()
                    ...
                    # Bomb cooldown: ✅ sadece symbol bazlı
                    if "BOMB" in stype:
                    ...
                    # 5) Bomb gönderildiyse bomb_history güncelle
                    if "BOMB" in stype:
                        bomb_history[symbol] = utc_now

                    # 6) Bu sinyali gönderiyoruz → cooldown yaz
                    ...
                    compression = np.nan
                    try:
                        cclose = float(last.get("close", np.nan))
                        a = float(last.get("atr", np.nan))
                        if cclose and not np.isnan(a) and not np.isnan(cclose):
                            atr_pct_val = (a / cclose) * 100.0
                    except Exception:
                        pass
                    try:
...
"""
                    ...
                    if not TELEGRAM_TOKEN or not CHAT_ID:
                        reject["telegram_missing_env"] += 1
                        print("⚠️ Telegram ENV eksik (TELEGRAM_TOKEN / TELEGRAM_CHAT_ID). Mesaj gönderilmedi.", flush=True)
                        continue
                    ...
                    try:
                        ...
                        print(f"✅ SİNYAL (L2): {symbol} | {data['type']}", flush=True)
                    ...
                    print(f"⚠️ L2 Hata ({symbol}): {e}", flush=True)
                    continue
            ...
            print("━━━━━━━━━━━━━━━━━━━━", flush=True)
            print(f"🧲 Aday (L1)                           : {reject.get('candidates', 0)}", flush=True)
            print(f"✅ Gönderilen (L2)                     : {reject.get('sent', 0)}", flush=True)

            if reject.get("cooldown", 0):
                print(f"⏳ Cooldown                            : {reject['cooldown']}", flush=True)
            ...
            if reject.get("risk_gt_target", 0):
                print(f"🧮 Risk > Target                       : {reject['risk_gt_target']}", flush=True)
            if reject.get("bomb_cooldown", 0):
                print(f"💣 Bomb Cooldown                       : {reject['bomb_cooldown']}", flush=True)
            if reject.get("telegram_fail", 0):
                print(f"📨 Telegram Hatası                     : {reject['telegram_fail']}", flush=True)
            if reject.get("telegram_missing_env", 0):
                print(f"📨 Telegram ENV Eksik                  : {reject['telegram_missing_env']}", flush=True)
            ...
            # bir sonraki 15m kapanışını bekle
            wait_until_next_15m_close()
...
if __name__ == "__main__":
    ...
    print(f"✅ Katman-2 Delay: {L2_DELAY_SEC}s / aday", flush=True)
...
PULLBACK_COOLDOWN_MIN = 360
REACCU_COOLDOWN_MIN = 480
L2_DELAY_SEC = 55  # ✅ YENİ: Katman-2 gecikmesi (coin başı 55 sn)
...
def wait_until_next_15m_close():
    while True:
        ...
        if next_multiple >= 60:
            next_multiple = 0
            now = now + timedelta(hours=1)
        next_close = now.replace(minute=next_multiple, second=2, microsecond=0)
        sleep_sec = (next_close - datetime.now(timezone.utc)).total_seconds()
        if sleep_sec > 0:
            tr_time = next_close.astimezone(timezone(timedelta(hours=3))).strftime('%H:%M:%S')
            print(f"😴 Sonraki 15m kapanış bekleniyor: {tr_time} TR ({sleep_sec:.0f}s)", flush=True)
...
# --- TEMEL FONKSİYONLAR (fmt_price, _fmt_num, vs.) ---
def fmt_price(symbol: str, price) -> str:
    try:
        if price is None:
            return "N/A"
...
# ✅ DÜZELTİLMİŞ: Parametre adı 'data' (dict değil)
...
# --- KALAN FONKSİYONLAR (get_data, prepare_indicators, vs.) ---
app = Flask(__name__)
signal_history = {}
bomb_history = {}
BOMB_COOLDOWN = timedelta(hours=24)
...
def home():
    now = datetime.now(timezone(timedelta(hours=3))).strftime('%H:%M:%S')
    return f"""
    <h1>🚀 Sniper Bot v6.0 (2 Katmanlı)</h1>
    <p><b>Durum:</b> {bot_status['status']}</p>
    <p><b>Son Tarama:</b> {bot_status['last_run']}</p>
    <p><b>Katman-1:</b> Tüm coin'lerde 15m sinyal arama</p>
    <p><b>Katman-2:</b> Sadece adaylarda 1h+spread+liquidity</p>
    <p><b>Ban Riski:</b> ✅ DÜŞÜK</p>
    """
...
# ✅ YENİ: 2 KATMANLI SİSTEM
def run_bot_engine():
    print("🚀 Sniper Bot v6.0 BAŞLATILDI (2 Katmanlı Sistem)", flush=True)
    bot_status["status"] = "Aktif"
    wait_until_next_15m_close()
    ...
    while True:
        try:
            ...
            reject = defaultdict(int)
            sent_by_type = defaultdict(int)
            candidates = []  # ✅ YENİ: Katman-1'de bulunan adaylar
            ...
            # ✅ KATMAN-1: Tüm coin'lerde HAFİF tarama (sadece 15m)
            for symbol in symbols:
                count += 1
                utc_now = datetime.now(timezone.utc)
                tr_time = utc_now.astimezone(timezone(timedelta(hours=3)))
                ...
                try:
                    ...
                    # ✅ Sadece 15m ile sinyal arama (ağır kontroller YOK)
                    signal_found = False
                    data = {}
                    ...
                    # ✅ Katman-1'de sadece aday listesine ekle (ağır kontroller YOK)
            ...
            # ✅ KATMAN-2: Sadece adaylarda AĞIR doğrulama
            ...
            if candidates:
                for idx, (symbol, data, df_15m, coin_regime, macro_regime) in enumerate(candidates, 1):
                    ...
                    time.sleep(L2_DELAY_SEC)  # ✅ YENİ: Coin başı gecikme
                    
                    try:
                        # ✅ Ağır kontroller BURADA çalışıyor (sadece adaylarda)
                        spread_ok, spread_msg = check_spread_safety(symbol)
                        if not spread_ok:
                            reject["spread_risk"] += 1
...
if __name__ == "__main__":
    ...
    wd.start()
    print("🌍 Web Sunucusu Başladı", flush=True)
    print("✅ Katman-1: Tüm coin'lerde 15m sinyal arama", flush=True)
    print("✅ Katman-2: Sadece adaylarda 1h+spread+liquidity", flush=True)
    print("✅ Ban Riski: %95 azaltılmış", flush=True)
    run_bot_engine()
