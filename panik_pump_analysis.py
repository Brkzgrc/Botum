#!/usr/bin/env python3
"""
PANİK PUMP — ters filtre + çoklu eşik analizi
Ters filtreler: büyük panik ve büyük kümülatif düşüşleri ELE
"""
...
def fetch_ohlcv(symbol, open_time_ms, interval, limit):
    ...
    try:
        ...
        if not klines:
            return None
        return {"opens": [float(k[1]) for k in klines],
...
def row(records, label, fn):
    ...
    if not combo:
        print(f"  {label:<28} sinyal yok", flush=True)
        return
    ...
    print(f"  {label:<28} {cw}/{len(combo):2d} → WR %{wr:2.0f}  {mark}  (eliyor {eli}/{total})", flush=True)
...
def main():
    ...
    for sig in panik:
        ...
        # Ters filtreler — büyük düşüşleri ELE
        f4t_neg = (candle_drop <= 0)    if candle_drop  is not None else None  # yeşil/nötr mum
        ...
        f5t_neg = (cum_drop <= 0)       if cum_drop     is not None else None  # yükselen 4 bar
        ...
        print(f"  {tag} {sym_short:8} | {pnl:+6.1f}% | {cr} {br} {cd} {cm}", flush=True)
    ...
    print(f"\n── TERS: KÜMÜLATİF DÜŞÜŞ {'─'*38}", flush=True)
    ...
    print(f"\n── KOMBİNASYONLAR {'─'*46}", flush=True)
    # F5t varyantları tek başına + F2/F3a ile
    row(records, "F5t_4 + F3a",              lambda r: r.get("f5t_4") and r.get("f3a"))
    row(records, "F5t_4 + F2",               lambda r: r.get("f5t_4") and r.get("f2"))
    row(records, "F5t_4 + F2 + F3a",         lambda r: r.get("f5t_4") and r.get("f2") and r.get("f3a"))
    row(records, "F5t_2 + F3a",              lambda r: r.get("f5t_2") and r.get("f3a"))
    row(records, "F5t_2 + F2",               lambda r: r.get("f5t_2") and r.get("f2"))
    row(records, "F5t_2 + F2 + F3a",         lambda r: r.get("f5t_2") and r.get("f2") and r.get("f3a"))
    row(records, "F5t_6 + F2 + F3a",         lambda r: r.get("f5t_6") and r.get("f2") and r.get("f3a"))
    # F4t varyantları
    row(records, "F4t_neg + F3a",            lambda r: r.get("f4t_neg") and r.get("f3a"))
    row(records, "F4t_neg + F2",             lambda r: r.get("f4t_neg") and r.get("f2"))
    row(records, "F4t_neg + F5t_4",          lambda r: r.get("f4t_neg") and r.get("f5t_4"))
    row(records, "F4t_neg + F5t_4 + F3a",    lambda r: r.get("f4t_neg") and r.get("f5t_4") and r.get("f3a"))
    row(records, "F4t_10 + F5t_4",           lambda r: r.get("f4t_10") and r.get("f5t_4"))
    row(records, "F4t_10 + F5t_4 + F3a",     lambda r: r.get("f4t_10") and r.get("f5t_4") and r.get("f3a"))
    row(records, "F4t_15 + F5t_4",           lambda r: r.get("f4t_15") and r.get("f5t_4"))
    row(records, "F4t_15 + F5t_4 + F2",      lambda r: r.get("f4t_15") and r.get("f5t_4") and r.get("f2"))
    row(records, "F4t_15 + F5t_4 + F2 + F3a",lambda r: r.get("f4t_15") and r.get("f5t_4") and r.get("f2") and r.get("f3a"))
    # Önceki en iyisi karşılaştırma için
