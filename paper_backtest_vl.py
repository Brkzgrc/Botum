...
Kullanım:
  python paper_backtest_vl.py              # Cache varsa kullan
  python paper_backtest_vl.py --no-fetch  # Sadece cache
  python paper_backtest_vl.py --coins BTC ETH SOL

NOT: 2022 verisi için eski cache silinip yeniden indirilmeli.
     rm -rf backtest_data/ && python paper_backtest_vl.py
"""
...
def load_or_fetch(symbol):
    df = load_pkl(symbol)
    if df is not None:
        # Cache 2022'den önce başlamıyorsa yeniden indir
