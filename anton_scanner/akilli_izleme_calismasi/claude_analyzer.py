...
_TYPE_NAMES = {
    ...
    "smart_watch": "AKILLI İZLEME — çok saatli fiyat hikâyesi ve retest/rol değişimi incelemesi",
}
_SOURCE_NAMES = {
    "bot":   "Pump Scanner Bot",
    "smc":   "SMC Sistemi",
    "smc-v2": "SMC CHoCH ROC Sistemi",
    "spot-scanner": "Akıllı Spot İzleme",
}
_TYPE_SHORT = {
    "capit":    "PANİK PUMP",
    "t72":      "ORTA VADE",
...
def _build_sig_data(signal: dict) -> str:
    ...
    skip = {"symbol","type","entry","stop","tp1","tp2","tp3","source","_internal","chart_png_base64"}
...
def evaluate(signal: dict, recent_count: int = 0) -> tuple[str, dict]:
    ...
    source   = signal.get("source", "bot")
    chart_png_base64 = signal.get("chart_png_base64", "") if sig_type == "smart_watch" else ""
...
UYARI: (varsa 1 cümle, yoksa yazma)"""
    ...
    if sig_type == "smart_watch":
        watch_fields = {
            key: signal.get(key) for key in (
                "action_state", "story_score", "price_story", "trigger_text",
                "invalidation_text", "extension_state", "market_regime",
                "target_pct", "stop_pct", "rr", "tf_summary", "positives",
                "risks", "swing_sequence", "support_interaction", "role_flip",
                "price_structure_1h", "indicator_phases_1h", "indicator_phases_4h",
            )
        }
        prompt = f"""Bir kripto spot traderı gibi bağımsız grafik ve fiyat davranışı değerlendirmesi yap.
Bu bir AL sinyali değil; 100+ coin içinden ön elemeye takılan bir İZLEME adayıdır.
Sistemin verdiği action_state kararını doğru kabul etme: verilerle doğrula veya reddet.

Şunları özellikle ayır:
- Sağlıklı güçlü trend ile geç kalınmış/parabolik fiyat
- Kırılan direncin gerçekten desteğe dönüşmesi ile sahte kırılım
- Tek fitil ile birkaç saat süren satıcı başarısızlığı
- Şimdi giriş, retest bekleme, yalnız izleme ve tamamen geçersiz senaryolar

[ADAY]
Coin: #{symbol.replace('/USDT','')}
Fiyat: {_fmt(signal.get('entry'))} | Yapısal stop: {_fmt(signal.get('stop'))} | İlk direnç: {_fmt(signal.get('tp1'))}
Scanner hikâyesi: {json.dumps(watch_fields, ensure_ascii=False, default=str)}

[BAĞIMSIZ ÇOKLU ZAMAN DİLİMİ VERİSİ]
{coin_block}

[BTC VE PİYASA]
{btc_block}
BTC makro: {macro_block if macro_block else 'veri yok'}
Fear & Greed: {fg_str}
{_dom_str(dom)}
{sweep_str}

Yalnız veride bulunan davranışları kullan; uydurma formasyon yazma. Her durumda giriş önermek zorunda değilsin.

KARAR: [✅ GİRİŞ TETİĞİ / 🟡 RETEST BEKLE / 🔎 YALNIZ İZLE / 🚫 GEÇERSİZ-KOVALAMA]
FİYAT HİKÂYESİ: (2-3 somut cümle)
GİRİŞ DAVRANIŞI: (hangi fiyat davranışı görülürse değerlendirilebilir)
GEÇERSİZLİK: (hangi kapanış veya yapı fikri bozar)
EK RİSK: (varsa tek cümle)"""
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if chart_png_base64:
                # Üst panel 1H, alt panel 4H; sarı/turuncu/kırmızı EMA20/50/200,
                # mavi destek, mor direnç ve kırmızı yapısal stop çizgisidir.
                message_content = [
                    {"type": "image", "source": {
                        "type": "base64", "media_type": "image/png", "data": chart_png_base64,
                    }},
                    {"type": "text", "text": (
                        "Grafik görseli: üst panel 1H, alt panel 4H. "
                        "Sarı/turuncu/kırmızı çizgiler EMA20/50/200; mavi destek, "
                        "mor direnç, kırmızı stop referansıdır.\n\n" + prompt
                    )},
                ]
            resp = client.messages.create(
                model="claude-haiku-4-5-20251001",
...
def _extract_verdict(decision: str) -> str:
    if "✅" in decision: return "✅ GİRİŞ TETİĞİ" if "TETİĞ" in decision else "✅ GİR"
    if "🟡" in decision: return "🟡 RETEST BEKLE"
    if "🔎" in decision: return "🔎 YALNIZ İZLE"
