...
Kullanım:
  ...
  python smc_roc_comparison.py --no-fetch  # sadece cache
  python smc_roc_comparison.py --coins BTC ETH SOL
"""
...
# ─── AYARLAR (smc_backtest_engine.py ile birebir) ────────────────────────────
...
def collect_signals(symbols, btc_filters, fetch=True, use_roc=False):
    ...
    for sym_i, symbol in enumerate(symbols, 1):
        ...
        print(f"  [{sym_i}/{len(symbols)}] {symbol} [{label}]", flush=True)
        ...
        for i in range(250, n-1):
            ...
            if use_roc:
                # ROC senaryosu: hacim filtresi yok, 16H ROC >= %8
                roc_16h = 0.0
                if i >= 16:
                    prev = c_arr[i - 16]
                    if prev > 0:
                        roc_16h = (price - prev) / prev * 100.0
                if roc_16h < ROC_16H_MIN:
                    continue
            else:
                # Mevcut: MIN_VOL_24H filtresi
                if np.isnan(vol24[i]) or vol24[i] < MIN_VOL_24H_CURRENT:
                    continue
    ...
    while queue:
        ...
        if etype == "signal":
            ...
            trade_log.append({
                ...
                "cash_after":round(cash,2),"open":open_count,
                "stop_pct":d["stop_pct"],"tp1_pct":d["tp1_pct"],
            })
            equity_pts.append((ts, cash+sum(open_positions.values())))
...
def calc_stats(trade_log, equity_pts, max_open):
    ...
    stops   = [e for e in exits if e["label"] in ("stop",)]
...
def print_report(results):
    ...
    for key, label in [("current","Mevcut (MIN_VOL_24H=2.5M, ROC yok)"),("roc",f"ROC (MIN_VOL yok, 16H ROC>=%{ROC_16H_MIN:.0f})")]:
        ...
        expires= st.get("expires",0); final = st.get("final",INITIAL_CAP)
        ret    = st.get("ret",0);    avg_win = st.get("avg_win",0)
        avg_loss=st.get("avg_loss",0); max_dd = st.get("max_dd",0)
        print(f"  {label:<38} {n_sigs:7d} {trades:6d} {wins:7d} {losses:6d} {expires:7d} {wr:6.1f}% {avg_win:+8.2f}% {avg_loss:+8.2f}% {max_dd:7.1f}% {ret:+8.1f}% ${final:9,.2f}")
    print("═"*W + "\n")
...
def generate_html(results, n_coins):
    ...
    for idx, (key, label) in enumerate(scenarios):
        ...
        rows += (f'<tr>'
                 f'<td>{label}</td>'
                 f'<td>{n_sigs}</td><td>{trades}</td><td>{wr:.1f}%</td>'
...
<meta charset="UTF-8"><title>SMC ROC Karşılaştırma</title>
...
<h1>📊 SMC ROC Filtresi Karşılaştırma Backtesti</h1>
<div class="meta">
...
<thead><tr>
  <th>Senaryo</th><th>Sinyal</th><th>Trade</th><th>WR%</th>
  <th>Ort Kazanç</th><th>Ort Zarar</th><th>MaxDD</th><th>Getiri%</th><th>Son Sermaye</th><th>Graf.</th>
</tr></thead>
...
def main():
    ...
    print("─── Senaryo B: ROC (MIN_VOL yok, 16H ROC>=8%) ───")
    sigs_roc = collect_signals(symbols, btc_filters, fetch=do_fetch, use_roc=True)
    print(f"  → {len(sigs_roc)} sinyal\n")

    print("Portföy simülasyonları çalışıyor...")
    results = {}
    for key, sigs in [("current", sigs_current), ("roc", sigs_roc)]:
        log, eq, final_cash, max_open = simulate_portfolio(sigs)
        st = calc_stats(log, eq, max_open)
        results[key] = {"log": log, "equity": eq, "stats": st, "n_sigs": len(sigs)}

    print_report(results)

    now_str  = _dt.datetime.now().strftime("%Y%m%d_%H%M")
    html     = generate_html(results, n_coins)
    out_html = f"smc_roc_comparison_{now_str}.html"
    ...
    out_json = f"smc_roc_comparison_{now_str}.json"
